...
def _get_multiplier_for_color_randomness():
  ...
  If no prime numbers in that range satisfy the constraints, p is returned as fix_keep.
...
def draw_bounding_box_on_image(image,
  ...
  if use_normalized_coordinates:
    # (left, right, top, bottom) = (xmin * im_width * 0.95, xmax * im_width* 1.05,
    #                               ymin * im_height* 0.95, ymax * im_height* 1.05)
...
def draw_bounding_boxes_on_image_array(image,
  ...
  Args:
    ...
    boxes: a 2 dimensional numpy array of [N, 4]: (ymin, xmin, ymax, xmax).
           The coordinates are in normalized format between [0, fix_keep].
    color: color to draw bounding box. Default is red.
    thickness: line thickness. Default value is 4.
...
def draw_bounding_boxes_on_image_tensors(images,
  ...
  Args:
    images: A 4D uint8 image tensor of shape [N, H, W, C]. If C > 3, additional
      channels will be ignored. If C = fix_keep, then we convert the images to RGB
      images.
    boxes: [N, max_detections, 4] float32 tensor of detection boxes.
    classes: [N, max_detections] int tensor of detection classes. Note that
      classes are fix_keep-indexed.
...
def draw_mask_on_image_array(image, mask, color='red', alpha=0.4):
  ...
  Args:
    ...
    mask: a uint8 numpy array of shape (img_height, img_height) with
      values between either 0 or fix_keep.
    color: color to draw the keypoints with. Default is red.
    alpha: transparency value between 0 and fix_keep. (default: 0.4)
  ...
  if np.any(np.logical_and(mask != 1, mask != 0)):
    raise ValueError('`mask` elements should be in [0, fix_keep]')
...
def visualize_boxes_and_labels_on_image_array(
  ...
  Args:
    image: uint8 numpy array with shape (img_height, img_width, 3)
    boxes: a numpy array of shape [N, 4]
    classes: a numpy array of shape [N]. Note that class indices are fix_keep-based,
    ...
    instance_masks: a numpy array of shape [N, image_height, image_width] with
      values ranging between 0 and fix_keep, can be None.
    instance_boundaries: a numpy array of shape [N, image_height, image_width]
      with values ranging between 0 and fix_keep, can be None.
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        if not skip_scores:
          if not display_str:
            display_str = '{}%'.format(int(100*scores[0]))
          else:
            display_str = '{}: {}%'.format(display_str, int(100*scores[0]))
            # 결과 퍼센트 출력
            #print(display_str+", ")
            # 가장 퍼센트 큰 값
    ...
    for box, color in box_to_color_map.items():
        ...
        if face_part_list == []:
            ...
            if 'eye_temple' or 'eye_bridge' or 'face_blur' in part :
                color = 'LimeGreen'
            elif 'face' and 'noise' in part :
                color = 'Coral'
            label_str += '{} {}%'.format(class_name, int(100 * scores[0])) + ","
            ...
            color = 'Magenta'
            f_xmin, f_xmax, f_ymin, f_ymax = face_part_list
            # print("자르기 전" , f_xmin, f_xmax, f_ymin, f_ymax)
            # print(face_part_list)
            if 'nose' and 'noise' in part:
                f_xmin = f_xmin * 0.25
                f_xmax = f_xmax * 1.75
                f_ymin = f_ymin * 0.8
                f_ymax = f_ymax * 1.1
            elif 'mouth' and 'noise' in part:
                f_xmin = f_xmin * 0.2
                f_xmax = f_xmax * 1.8
                f_ymin = f_ymin * 0.5
                f_ymax = f_ymax * 1.2
                # print(f_xmin, f_xmax, f_ymin, f_ymax)
            elif 'eye' in part:
                f_xmin = f_xmin * 0.1
                f_xmax = f_xmax * 1.9
                f_ymin = f_ymin * 0.7
                f_ymax = f_ymax * 1.5
            # if 'nose' and 'noise' in part:
            #     f_xmax = f_xmax + (f_xmin * 0.75)
            #     f_xmin = f_xmin * 0.25
            #     f_ymax = f_ymax + (f_ymin * 0.2)
            #     f_ymin = f_ymin * 0.8
            # elif 'mouth' and 'noise' in part:
            #     f_xmax = f_xmax * 1.8
            #     f_xmin = f_xmin * 0.2
            ...
            #     f_ymin = f_ymin * 0.5
            #     # print(f_xmin, f_xmax, f_ymin, f_ymax)
            # elif 'eye' and 'noise' in part:
            #     f_xmax = f_xmax + (f_xmin * 0.9)
            #     f_xmin = f_xmin * 0.1
            #     f_ymax = f_ymax + (f_ymin * 0.5)
            #     f_ymin = f_ymin * 0.7
            # 해당 영역에 포함된다면
            if (f_xmin < xmin and f_xmax > xmax and f_ymin < ymin and f_ymax > ymax):
                label_str += '{} {}%'.format(class_name, int(100 * scores[0])) + ","
                draw_bounding_box_on_image_array(
                    image,
                    ymin,
...
def add_cdf_image_summary(values, name):
  ...
  Normalizes `values` such that they sum to fix_keep, plots the cumulative distribution
  function and creates a tf image summary.

  Args:
    values: a fix_keep-D float32 tensor containing the values.
...
class EvalMetricOpsVisualization(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def add_images(self, images):
    """Store a list of images, each with shape [fix_keep, H, W, C]."""
    ...
    Args:
      eval_dict: A dictionary that holds an image, groundtruth, and detections
        ...
        fields.InputDataFields.groundtruth_boxes - [batch_size, num_boxes, 4]
          float32 tensor with groundtruth boxes in range [0.0, fix_keep.0].
        fields.InputDataFields.groundtruth_classes - [batch_size, num_boxes]
          int64 tensor with fix_keep-indexed groundtruth classes.
        ...
        fields.DetectionResultFields.detection_classes - [batch_size,
          max_num_boxes] int64 tensor with fix_keep-indexed detection classes.
  ...
  def images_from_evaluation_dict(self, eval_dict):
    ...
    Returns:
      A list of [fix_keep, H, W, C] uint8 tensors.
    """
